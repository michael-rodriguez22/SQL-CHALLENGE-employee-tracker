...
const queries = {
  ...
  delete: {
    department: answers =>
      `DELETE FROM departments WHERE id = ${answers.department.match(idOnly)}`,
    role: answers =>
      `DELETE FROM roles WHERE id = ${answers.role.match(idOnly)}`,
    employee: answers =>
      `DELETE FROM employees WHERE id = ${answers.employee.match(idOnly)}`,
  },
}
